...
#ifndef FAAT_PCL_REC_FRAMEWORK_FPFH_LOCAL_ESTIMATOR_OMP_H_
#define FAAT_PCL_REC_FRAMEWORK_FPFH_LOCAL_ESTIMATOR_OMP_H_
...
namespace faat_pcl
{
  namespace rec_3d_framework
  {
    template<typename PointInT, typename FeatureT>
      class FAAT_3D_FRAMEWORK_API FPFHLocalEstimationOMP : public LocalEstimator<PointInT, FeatureT>
      {
      ...
      public:
                   ...
                   FeatureTPtr & signatures)
        {
          ...
          assert (in->points.size () == normals->points.size ());

          //compute signatures
          typedef typename pcl::FPFHEstimationOMP<PointInT, pcl::Normal, pcl::FPFHSignature33> FPFHEstimator;
          typename pcl::search::KdTree<PointInT>::Ptr tree (new pcl::search::KdTree<PointInT>);

          pcl::PointIndicesPtr indices_pcl;
          indices_pcl.reset(new pcl::PointIndices());
          ...
          fpfh_estimate.setNumberOfThreads(8);
          fpfh_estimate.setSearchMethod (tree);
          fpfh_estimate.setInputCloud (in);
          fpfh_estimate.setSearchSurface(in);
          fpfh_estimate.setIndices(indices_pcl);
          fpfh_estimate.setInputNormals (normals);
          fpfh_estimate.setRadiusSearch (support_radius_);
          fpfh_estimate.compute (*fpfhs);
