...
faat_pcl::rec_3d_framework::ModelOnlySource<Full3DPointT, PointInT>::computeFaces (ModelT & model)
{
    ...
    /*for(size_t i=0; i < supervoxel_clusters.size(); i++)
    {
        for(size_t j=0; j < supervoxel_clusters.size(); j++)
        {
            std::cout << (int)adjacent[i][j] << " ";
        }
...
template class faat_pcl::rec_3d_framework::ModelOnlySource<struct pcl::PointXYZRGBNormal, struct pcl::PointXYZRGB>;
template class faat_pcl::rec_3d_framework::ModelOnlySource<struct pcl::PointXYZ, struct pcl::PointXYZ>;
