...
int keyframesOnly;
int usePclRenderer;
bool enableCheckerboardRegistration = false;
...
void segmentation();
void globalRegistration2();
void nmBasedCloudIntegration2();
...
int main(int argc, char *argv[] )
{
    ...
    //boost::tuples::tuple<TestResult<int>*, TestResult<std::string>* > t;

    /*
    object_modeller::TestInput testInput;
    object_modeller::TestOutput testOutput;
    object_modeller::TestProcessor testProcessor;
    object_modeller::TestComplexProcessor testComplexProcessor;
    object_modeller::TestComplexResultProcessor testComplexResultProcessor;
    object_modeller::TestComplexOutput testComplexOut;

    std::string result = testInput.process();
    int result2 = testProcessor.process(result);
    int result3 = testComplexProcessor.process(boost::tuples::make_tuple(result, result2));
    testOutput.process(result3);

    boost::tuples::tuple<std::string, int> complexResult = testComplexResultProcessor.process(result);
    testComplexOut.process(complexResult);
    testComplexOut.process(boost::tuples::make_tuple(result, result2));

    object_modeller::TestPipeline testPipeline;


    object_modeller::TestResult<std::string> *r = testPipeline.addIn(&testInput);
    object_modeller::TestResultBase<int> *r2 = testPipeline.addInOut(&testProcessor, r);
    object_modeller::TestResultBase<int> *r3 = testPipeline.addInOut(&testComplexProcessor, r, r2);
    testPipeline.addOut(&testOutput, r3);

    object_modeller::TestResult<boost::tuples::tuple<std::string, int> > *cr = testPipeline.addInOut(&testComplexResultProcessor, r);
    testPipeline.addOut(&testComplexOut, cr);
    testPipeline.addOut(&testComplexOut, r, r2);

    object_modeller::TestResultBase<int> *foo = cr->get<1>();
    testPipeline.addOut(&testOutput, cr->get<1>());

    testPipeline.process();

    return 0;
    */

    //object_modeller::reader::FileReader::Ptr reader( new object_modeller::reader::FileReader(sourceDir, ".*cloud_.*.pcd"));
    //object_modeller::util::Transform::Ptr transform( new object_modeller::util::Transform());
    //object_modeller::registration::CameraTracker::Ptr cameraTracker( new object_modeller::registration::CameraTracker() );
    //object_modeller::output::TomGineRenderer::Ptr renderer( new object_modeller::output::TomGineRenderer() );

    // input reader
    object_modeller::reader::FileReader reader(sourceDir, ".*cloud_.*.pcd");

    // point cloud utility
    object_modeller::util::DistanceFilter distanceFilter(zDist);
    object_modeller::util::Transform transform;
    object_modeller::util::Mask mask;
    object_modeller::util::MultiplyMatrix multiply;

    // registration
    object_modeller::registration::CameraTracker cameraTracker;

    std::vector<cv::Size> checkerboardSizes;
    checkerboardSizes.push_back(cv::Size(4,5));
    checkerboardSizes.push_back(cv::Size(3,6));
    object_modeller::registration::CheckerboardRegistration checkerboard(checkerboardSizes);

    object_modeller::registration::GlobalRegistration globalRegistration;

    //segmentation
    object_modeller::segmentation::DominantPlaneExtraction dominantPlaneExtraction;

    //modelling
    object_modeller::modelling::NmBasedCloudIntegration nmBasedCloudIntegration;
    object_modeller::modelling::PoissonReconstruction poissonReconstruction;

    //renderer

    boost::shared_ptr<object_modeller::output::Renderer> renderer;

    //output
    std::stringstream temp;
    temp << sourceDir << "/out";
    object_modeller::output::PointCloudWriter pointCloudWriter(temp.str());
    object_modeller::output::IndicesWriter indicesWriter(temp.str());
    object_modeller::output::PosesWriter posesWriter(temp.str());


    if (usePclRenderer)
    {
        renderer.reset( new object_modeller::output::PclRenderer());
    } else {
        renderer.reset( new object_modeller::output::TomGineRenderer());
    }
    object_modeller::output::MeshRenderer meshRenderer(renderer);



    object_modeller::TestPipeline pipeline;

    //read from file
    object_modeller::reader::FileReader::ResultType *pointclouds_input = pipeline.addIn(&reader);

    //pipeline.add(checkerboard.cast_out(), reader.cast_in());

    // filter far points
    //pipeline.add(distanceFilter.cast_out(), reader.cast_in());
    object_modeller::util::DistanceFilter::ResultType *pointclouds_filtered = pipeline.addInOut(&distanceFilter, pointclouds_input);

    // TODO: keyframes

    object_modeller::registration::CameraTracker::ResultType *poses;

    if (enableCheckerboardRegistration)
    {
        poses = pipeline.addInOut(&checkerboard, pointclouds_filtered);
    }
    else
    {
        poses = pipeline.addInOut(&cameraTracker, pointclouds_filtered);
    }

    object_modeller::util::Transform::ResultType *pointclouds_transformed = pipeline.addInOut(&transform, pointclouds_filtered, poses);
    pipeline.addOut(renderer.get(), pointclouds_transformed, new object_modeller::TestResult<std::string>("Camera Tracker Output"), new object_modeller::TestResult<bool>(step));

    // TODO: keyframes
    //Checkerboard + transform + display
    /*
    pipeline.add(checkerboard.cast_out(), distanceFilter.cast_in());
    pipeline.add(transform.cast_out(), distanceFilter.cast_in(), checkerboard.cast_in());
    pipeline.add(renderer->cast_out(), transform.cast_in(), object_modeller::output::RendererArgs("Checkerboard registration", step));
    */

    //object_modeller::registration::CheckerboardRegistration::ResultPtr reg = pipeline.addIO(checkerboard.cast_io(), &filtered);
    //object_modeller::util::Transform::ResultPtr result = pipeline.add(transform.cast_io(), object_modeller::util::Transform::InputType::make_result(&filtered, &reg));
    //pipeline.add(renderer->cast_out(), transform.cast_in(), object_modeller::output::RendererArgs("Checkerboard registration", step));

    object_modeller::segmentation::DominantPlaneExtraction::ResultType *indices = pipeline.addInOut(&dominantPlaneExtraction, pointclouds_filtered);
    // segmentation + mask + display
    object_modeller::util::Mask::ResultType *pointclouds_segmented = pipeline.addInOut(&mask, pointclouds_transformed, indices);
    pipeline.addOut(renderer.get(), pointclouds_segmented, new object_modeller::TestResult<std::string>("Segmentation output"), new object_modeller::TestResult<bool>(step));

    //global registration

    if (enableGlobalRegistration)
    {
        object_modeller::registration::GlobalRegistration::ResultType *global_reg_poses = pipeline.addInOut(&globalRegistration, pointclouds_segmented);
        ...
        pipeline.addOut(renderer.get(), pointclouds_transformed, new object_modeller::TestResult<std::string>("Global registration output"), new object_modeller::TestResult<bool>(step));
    }

    // nm based cloud integration
    object_modeller::modelling::NmBasedCloudIntegration::ResultType *model = pipeline.addInOut(&nmBasedCloudIntegration, pointclouds_filtered, poses, indices);
    pipeline.addOut(renderer.get(), model, new object_modeller::TestResult<std::string>("NM based cloud integration output"), new object_modeller::TestResult<bool>(step));

    //pipeline.addOut(&pointCloudWriter, model);
    //pipeline.addOut(&indicesWriter, indices);
    //pipeline.addOut(&posesWriter, poses);

    object_modeller::modelling::PoissonReconstruction::ResultType *mesh = pipeline.addInOut(&poissonReconstruction, model);
    pipeline.addOut(&meshRenderer, mesh, new object_modeller::TestResult<std::string>("Poisson reconstruction output"), new object_modeller::TestResult<bool>(step));

    /*
    if (enableGlobalRegistration)
    {
        pipeline.add(nmBasedCloudIntegration.cast_out(), distanceFilter.cast_in(),
                 object_modeller::modelling::TransformAndMask(multiply.cast_in(), dominantPlaneExtraction.cast_in()));
    } else {
        pipeline.add(nmBasedCloudIntegration.cast_out(), distanceFilter.cast_in(),
                 object_modeller::modelling::TransformAndMask(cameraTracker.cast_in(), dominantPlaneExtraction.cast_in()));
    }

    pipeline.add(renderer->cast_out(), nmBasedCloudIntegration.cast_in(), object_modeller::output::RendererArgs("NM Based cloud integration", step));

    pipeline.add(pointCloudWriter.cast_out(), nmBasedCloudIntegration.cast_in());
    pipeline.add(indicesWriter.cast_out(), dominantPlaneExtraction.cast_in());
    pipeline.add(posesWriter.cast_out(), cameraTracker.cast_in());
...
void parseCommandLineArgs(int argc, char **argv)
{
    ...
    while(1)
    {
        c = getopt(argc, argv, "d:o:z:hsgkpc");
        ...
        switch(c)
        {
            ...
            case 'g':
                ...
                break;
            case 'c':
                enableCheckerboardRegistration = 1;
                break;
            case 'k':
                keyframesOnly = 1;
                break;
