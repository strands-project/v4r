...
#include <faat_framework/object_modelling/include/seg_do_modelling.h>
...
void
doSegmentation_intern (typename pcl::PointCloud<PointT>::Ptr & xyz_points,
                ...
                float min_height = 0.01f)
{
    ...
    if(seg_type == 0)
    {
        ...
        ref_comp->setDistanceThreshold (0.02f, true);
    ...
    else if(seg_type == 1)
    {
        ...
        dps.setObjectMinHeight (min_height);
...
{
    registerParameter("numPlaneInliers", "Number of Plane Inliers", &num_plane_inliers, 500);
    registerParameter("segType", "Segmentation type", &seg_type, 1);
    registerParameter("planeThreshold", "Plane Threshold", &plane_threshold, 0.01f);
}
...
std::vector<std::vector<int> > DominantPlaneExtraction::process(std::vector<pcl::PointCloud<pcl::PointXYZRGB>::Ptr> pointClouds)
{
    ...
    for (size_t i = 0; i < pointClouds.size (); i++)
    {
        std::vector<pcl::PointIndices> indices;
        Eigen::Vector4f table_plane;
        doSegmentation_intern<pcl::PointXYZRGB> (pointClouds[i], indices, table_plane, num_plane_inliers, seg_type, plane_threshold);
