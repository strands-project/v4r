...
/**
 * @brief Functions::isRunning
 * @return
 */
bool Functions::isRunning()
{
  return m_run;
}

/**
 * @brief Functions::cam_params_changed
 * @param _cam_params
 */
void Functions::cam_params_changed(const RGBDCameraParameter &_cam_params)
{
...
/**
 * @brief Functions::storeTrackingModel
 ...
 * @param _masks
 */
void Functions::storeTrackingModel(const std::string &_folder, const std::string &_objectname, const std::vector<Eigen::Matrix4f> &_cameras, const boost::shared_ptr< std::vector<std::pair<int, pcl::PointCloud<pcl::PointXYZRGB>::Ptr> > > &_clouds, const std::vector< cv::Mat_<unsigned char> > &_masks)
{
...
void Functions::run()
{
  ...
  case STORE_TRACKING_MODEL:
  {
    ...
    emit printStatus("Status: Start creating the tracking model...");
    createTrackingModel();
...
/**
 * @brief Functions::createTrackingModel
 */
void Functions::createTrackingModel()
{
  if (clouds->size() == 0)
    return;
  ...
  keyDet.reset(new v4r::FeatureDetector_KD_FAST_IMGD(v4r::FeatureDetector_KD_FAST_IMGD::Parameter(10000, 1.44, 2, 17)));
  keyDesc = keyDet;
...
/**
 * @brief Functions::saveTrackingModel
 */
void Functions::saveTrackingModel()
{
  if (model.get()==0)
      return;
 ...
 * @param model
 */
void Functions::addObjectView(const v4r::DataMatrix2D<Eigen::Vector3f> &cloud, const v4r::DataMatrix2D<Eigen::Vector3f> &normals, const cv::Mat_<unsigned char> &im, const cv::Mat_<unsigned char> &mask, const Eigen::Matrix4f &pose, v4r::ArticulatedObject &model)
{
...
/**
 * @brief Functions::detectCoordinateSystem
 * @param pose
 */
void Functions::detectCoordinateSystem(Eigen::Matrix4f &pose)
{
  pose.setIdentity();
  ...
  px = Eigen::Vector3f(pz[0], -pz[2], pz[1]);
  py = pz.cross(px);
