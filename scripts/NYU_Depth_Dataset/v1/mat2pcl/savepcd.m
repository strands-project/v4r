    ...
    switch nfields
        ...
        case 6
            ...
            if ascii
                ...
                points = [ points(1:3,:); double(typecast(rgb,'single'))];
            end
...
%             else
%                 points = [ points(1:3,:); double(typecast(rgba,'single'))];
%             end
            % RGB data
            RGB = uint32(points(4:6,:)*255);
            rgb = (RGB(1,:)*256+RGB(2,:))*256+RGB(3,:);
            ...
            if ascii
                points = [ points(1:3,:); double(rgb); points(7,:)];
            else               
                points = [ points(1:3,:); double(typecast(rgb,'single')); double(typecast(points(7,:),'uint32'))];
            end
    end
