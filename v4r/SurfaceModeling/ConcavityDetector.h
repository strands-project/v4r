...
/**
 * @file ConcavityDetector.h
 * @author Richtsfeld
 * @date October 2012
 * @version 0.1
 * @brief Detect concave corners in segments.
 */

#ifndef SURFACE_CONCAVITY_DETECTOR_H
#define SURFACE_CONCAVITY_DETECTOR_H
...
inline int ConcavityDetector::GetIdx(short x, short y)
{
  return y*pcl_cloud->width + x;
}

/** Return x coordinate for index **/
inline short ConcavityDetector::X(int idx)
{
  return idx%pcl_cloud->width;
}

/** Return y coordinate for index **/
inline short ConcavityDetector::Y(int idx)
{
  return idx/pcl_cloud->width;
}
