...
void BoundaryRelations::setInputCloud(pcl::PointCloud<pcl::PointXYZRGB>::Ptr _cloud)
{
...
void BoundaryRelations::setNormals(pcl::PointCloud<pcl::Normal>::Ptr _normals)
{
  if ( (_normals->height!=height) || (_normals->width!=width) )
    throw std::runtime_error("[ColorHistogram::setNormals] Invalid normals (not for this point cloud)");

  normals = _normals;
  have_normals = true;
}

void BoundaryRelations::createPatchImage(pcl::PointIndices::Ptr _indices, uchar idx, cv::Mat &patches)
{
  assert(patches.rows == height);
  assert(patches.cols == width);
  assert(patches.type() == CV_8UC1);

  // update patch image
  for(int i = 0; i < _indices->indices.size(); ++i)
  {
    int x = i % width;
    int y = i / width;

    patches.at<uchar>(y,x) = idx;
  }
}
