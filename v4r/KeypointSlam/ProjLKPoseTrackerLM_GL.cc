...
#include "ProjLKPoseTrackerLM_GL.hh"
 ...
 */
ProjLKPoseTrackerLM_GL::ProjLKPoseTrackerLM_GL(const Parameter &p)
 : param(p)
{ 
  plk.reset( new RefineProjectedPointLocationLKGPU(p.plk_param) );

  sqr_inl_dist = param.inl_dist*param.inl_dist;
}

ProjLKPoseTrackerLM_GL::~ProjLKPoseTrackerLM_GL()
{
}
 ...
 * getRandIdx
 */
void ProjLKPoseTrackerLM_GL::getRandIdx(int size, int num, std::vector<int> &idx)
{
  int temp;
  idx.clear();
  for (int i=0; i<num; i++)
 ...
 * countInliers
 */
unsigned ProjLKPoseTrackerLM_GL::countInliers(const std::vector<Eigen::Vector3d> &points, const std::vector<cv::Point2f> &im_points, const Eigen::Matrix4d &pose)
{
  unsigned cnt=0;
 ...
 * getInliers
 */
void ProjLKPoseTrackerLM_GL::getInliers(const std::vector<Eigen::Vector3d> &points, const std::vector<cv::Point2f> &im_points, const Eigen::Matrix4d &pose, std::vector<int> &inliers)
{
  Eigen::Vector2f im_pt;
  Eigen::Vector3d pt3;
  bool have_dist = !tgt_dist_coeffs.empty();
 ...
 * optimizePoseLM
 */
void ProjLKPoseTrackerLM_GL::optimizePoseLM(const std::vector<Eigen::Vector3d> &points, const std::vector<int> &pt_indices, const std::vector<cv::Point2f> &im_points, const std::vector<int> &im_indices, Eigen::Matrix4d &pose)
{
  Eigen::Matrix3d R = pose.topLeftCorner<3, 3>();
  Eigen::Vector3d t = pose.block<3,1>(0, 3);
 ...
 */
void ProjLKPoseTrackerLM_GL::optimizePoseRobustLossLM(const std::vector<Eigen::Vector3d> &points, const std::vector<int> &pt_indices, const std::vector<cv::Point2f> &im_points, const std::vector<int> &im_indices, Eigen::Matrix4d &pose)
{
  Eigen::Matrix3d R = pose.topLeftCorner<3, 3>();
  Eigen::Vector3d t = pose.block<3,1>(0, 3);
  Eigen::Matrix<double, 6, 1> pose_Rt;
 ...
 * ransacPoseLM
 */
void ProjLKPoseTrackerLM_GL::ransacPoseLM(const std::vector<Eigen::Vector3d> &points, const std::vector<cv::Point2f> &im_points, Eigen::Matrix4d &pose, std::vector<int> &inliers)
{
  int k=0;
 ...
 * detect
 */
double ProjLKPoseTrackerLM_GL::detect(const cv::Mat &image, Eigen::Matrix4f &pose)
{
  inliers.clear();

  if (model.get()==0)
    throw std::runtime_error("[ProjLKPoseTrackerLM_GL::detect] No model available!");
  if (src_intrinsic.empty()||tgt_intrinsic.empty())
    throw std::runtime_error("[ProjLKPoseTrackerLM_GL::detect] Intrinsic camera parameter not set!");
  ...
  for (unsigned z=0; z<im_points.size(); z++)
  {
    if (converged[z]==1)
    {
      ...
      model_pts.push_back(points[z].cast<double>());
 ...
 */
void ProjLKPoseTrackerLM_GL::getProjections(std::vector< std::pair<int,cv::Point2f> > &im_pts)
{
  im_pts.clear();
 ...
 * setModel
 */
void ProjLKPoseTrackerLM_GL::setModel(const ObjectView::Ptr &_model, const Eigen::Matrix4f &_pose) 
{  
  model=_model; 
 ...
 */
void ProjLKPoseTrackerLM_GL::setSourceCameraParameter(const cv::Mat &_intrinsic, const cv::Mat &_dist_coeffs)
{
 ...
 */
void ProjLKPoseTrackerLM_GL::setTargetCameraParameter(const cv::Mat &_intrinsic, const cv::Mat &_dist_coeffs)
{
