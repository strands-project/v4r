...
//    pcl::visualization::PointCloudColorHandlerRGBField<PointT> scene_handler(scene_cropped_to_model);
//    viewer.addPointCloud(scene_cropped_to_model, scene_handler, "scene");
...
//    pcl::transformPointCloud(*merged_cloud_for_refinement_start_, *cloud_voxelized_icp_cropped, pose_refined_);
    ...
    //            est (new faat_pcl::rec_3d_framework::VoxelBasedCorrespondenceEstimation<pcl::PointXYZRGB,  pcl::PointXYZRGB> ());

    //    pcl::registration::CorrespondenceRejectorSampleConsensus<pcl::PointXYZRGB>::Ptr
    //            rej ( new pcl::registration::CorrespondenceRejectorSampleConsensus< pcl::PointXYZRGB> ());
...
void MainWindow::save_model()
{
    ...
    if(gt_or_ouput_dir.compare("") != 0)
    {
        ...
        for (size_t i=0; i < single_scenes_.size(); i++)
        {
            ...
            scene_cloud->points.resize(single_scenes_[i]->points.size());
            for(size_t pt_id = 0; pt_id < single_scenes_[i]->points.size(); pt_id++)
            {
                scene_cloud->points[pt_id].x = single_scenes_[i]->points[pt_id].x;
                scene_cloud->points[pt_id].y = single_scenes_[i]->points[pt_id].y;
                scene_cloud->points[pt_id].z = single_scenes_[i]->points[pt_id].z;
            }
            ...
            for (size_t k=0; k < sequence_hypotheses_.size(); k++)
            {
                ...
                pose_file << gt_or_ouput_dir << "/" << scene << "_" << model_id_replaced << "_" << id_c_it->second << ".txt";
