...
void Recognizer::constructHypotheses()
{
        if(pSceneNormals_->points.size() == 0)
        {
            ...
            v4r::ORUtils::miscellaneous::computeNormals(pInputCloud_, pSceneNormals_, normal_computation_method_);
        }

        {
            pcl::visualization::PCLVisualizer vis("cloud");
            pcl::visualization::PointCloudColorHandlerRGBField<PointT> rgb_handler(pInputCloud_);
            vis.addPointCloud(pInputCloud_, rgb_handler, "original_cloud");
            vis.addPointCloudNormals<pcl::PointXYZRGB, pcl::Normal> (pInputCloud_, pSceneNormals_, 10, 0.05, "normals");
            vis.spin();
        }
...
void Recognizer::printParams() const
{
    std::cout << "cg_size_thresh: " << cg_params_.cg_size_threshold_ << std::endl
              ...
              << "icp_iterations: " << icp_iterations_ << std::endl
              << "icp_type: " << icp_type_ << std::endl
              << "icp_voxel_size: " << hv_params_.resolution_ << std::endl
              << "do_sift: " << do_sift_ << std::endl
              << "do_shot: " << do_shot_ << std::endl
              << "do_ourcvfh: " << do_ourcvfh_ << std::endl
              << "====================" << std::endl << std::endl;
}
...
//    model_only_source_->generate (test);
    ...
    if (do_sift_)
    {
      ...
      new_sift_local->setICPIterations (icp_iterations_);
...
//      new_sift_local_->setCGAlgorithm (cast_cg_alg);
    ...
    if(do_ourcvfh_ && USE_SEGMENTATION_)
    {
      ...
      rf_color_ourcvfh_global_->setICPIterations ( icp_iterations_ );
...
//        local->setCGAlgorithm (cast_cg_alg);
        ...
        local->setICPIterations ( icp_iterations_ );
