...
private:
    ...
    boost::shared_ptr<v4r::MultiviewRecognizer> r_;
    v4r::MultiviewRecognizer::MVParameter mv_params_;
...
public:
    ...
    bool initialize(int argc, char ** argv)
    {
        ...
        pcl::console::parse_argument (argc, argv,  "-scene_to_scene", mv_params_.scene_to_scene_);
        pcl::console::parse_argument (argc, argv,  "-hyp_to_hyp", mv_params_.hyp_to_hyp_);
        pcl::console::parse_argument (argc, argv,  "-use_robot_pose", mv_params_.use_robot_pose_);
        pcl::console::parse_argument (argc, argv,  "-extension_mode", mv_params_.extension_mode_);
        pcl::console::parse_argument (argc, argv,  "-max_vertices_in_graph", mv_params_.max_vertices_in_graph_);
        pcl::console::parse_argument (argc, argv,  "-distance_keypoints_get_discarded", mv_params_.distance_keypoints_get_discarded_);

        r_.reset(new v4r::MultiviewRecognizer(mv_params_));
...
//                    visualize_result(cloud, verified_models, transforms_verified);
                ...
                for(size_t m_id=0; m_id<verified_models.size(); m_id++)
                {
                    ...
                    std::cout << "******" << model_id << std::endl << tf_tmp << std::endl << std::endl;
                }
            }
        }
        return true;
