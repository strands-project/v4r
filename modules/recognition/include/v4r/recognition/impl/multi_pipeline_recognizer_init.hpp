...
MultiRecognitionPipeline<PointT>::MultiRecognitionPipeline(int argc, char **argv)
{
    ...
    desc.add_options()
            ...
            ("sift_knn", po::value<size_t>(&paramSiftRecognizer.knn_)->default_value(paramSiftRecognizer.knn_), "sets the number k of matches for each extracted SIFT feature to its k nearest neighbors")
            ("sift_use_codebook", po::value<bool>(&paramSiftRecognizer.use_codebook_)->default_value(false), "if true, performs K-Means clustering on all signatures being trained.")
            ("sift_codebook_size", po::value<size_t>(&paramSiftRecognizer.codebook_size_)->default_value(paramSiftRecognizer.codebook_size_), "number of clusters being computed for the codebook (K-Means)")
            ("sift_codebook_filter_ratio", po::value<float>(&paramSiftRecognizer.codebook_filter_ratio_)->default_value(paramSiftRecognizer.codebook_filter_ratio_), "signatures clustered into a cluster which occures more often (w.r.t the total number of signatures) than this threshold, will be rejected.")
            ("sift_filter_planar", po::value<bool>(&paramSiftRecognizer.filter_planar_)->default_value(false), "If true, filters keypoints which are on a planar surface..")
            ("sift_filter_points_above_plane", po::value<bool>(&paramSiftRecognizer.filter_points_above_plane_)->default_value(false), "If true, only recognizes points above dominant plane.")
            ("sift_filter_border_pts", po::value<bool>(&paramSiftRecognizer.filter_border_pts_)->default_value(paramSiftRecognizer.filter_border_pts_), "If true, filters keypoints at the boundary.")
            ("chop_z,z", po::value<float>(&sift_chop_z)->default_value(sift_chop_z), "points with z-component higher than chop_z_ will be ignored for SIFT (low chop_z reduces computation time and false positives (noise increase with z)")
            ...
            ("use_3d_model_for_training", po::value<bool>(&paramShotRecognizer.use_3d_model_)->default_value(paramShotRecognizer.use_3d_model_), "if true, it learns features directly from the reconstructed 3D model instead of on the individual training views.")
            ("shot_use_codebook", po::value<bool>(&paramShotRecognizer.use_codebook_)->default_value(paramShotRecognizer.use_codebook_), "if true, performs K-Means clustering on all signatures being trained.")
            ("shot_filter_planar", po::value<bool>(&paramShotRecognizer.filter_planar_)->default_value(true), "If true, filters keypoints which are on a planar surface.")
            ("shot_filter_points_above_plane", po::value<bool>(&paramShotRecognizer.filter_points_above_plane_)->default_value(true), "If true, only recognizes points above dominant plane.")
            ("shot_plane_threshold", po::value<float>(&paramShotRecognizer.threshold_planar_)->default_value(paramShotRecognizer.threshold_planar_), "Threshold ratio for planarity definition")
            ("shot_plane_support_radius", po::value<float>(&paramShotRecognizer.planar_support_radius_)->default_value(paramShotRecognizer.planar_support_radius_), "Patch size used to check keypoints for planarity")
            ...
            ("shot_filter_border_pts", po::value<bool>(&paramShotRecognizer.filter_border_pts_)->default_value(paramShotRecognizer.filter_border_pts_), "If true, filters keypoints at the boundary.")
            ("shot_border_width", po::value<int>(&paramShotRecognizer.boundary_width_)->default_value(paramShotRecognizer.boundary_width_), "Width in pixel of the depth discontinuity.")
            ("shot_visualize_keypoints", po::bool_switch(&paramShotRecognizer.visualize_keypoints_), "If set, visualizes the extracted 3D keypoints")
            ("shot_max_distance", po::value<double>(&paramShotRecognizer.max_distance_)->default_value(1.5f), "points with z-component higher than chop_z_ will be ignored for SHOT (low chop_z reduces computation time and false positives (noise increase with z)")
            ("shot_knn", po::value<size_t>(&paramShotRecognizer.knn_)->default_value(paramShotRecognizer.knn_), "sets the number k of matches for each extracted SHOT feature to its k nearest neighbors")
            ...
            ("shot_codebook_size", po::value<size_t>(&paramShotRecognizer.codebook_size_)->default_value(paramShotRecognizer.codebook_size_), "number of clusters being computed for the codebook (K-Means)")
            ("shot_codebook_filter_ratio", po::value<float>(&paramShotRecognizer.codebook_filter_ratio_)->default_value(paramShotRecognizer.codebook_filter_ratio_), "signatures clustered into a cluster which occures more often (w.r.t the total number of signatures) than this threshold, will be rejected.")
            ("shot_kernel_sigma", po::value<float>(&paramShotRecognizer.kernel_sigma_)->default_value(paramShotRecognizer.kernel_sigma_), "signatures clustered into a cluster which occures more often (w.r.t the total number of signatures) than this threshold, will be rejected.")
            ("sift_border_width", po::value<int>(&paramSiftRecognizer.boundary_width_)->default_value(paramSiftRecognizer.boundary_width_), "Width in pixel of the depth discontinuity.")
