...
void
PartialPCDSource<Full3DPointT, PointInT, OutModelPointT>::loadOrGenerate (const std::string & model_path, ModelT & model)
{
  ...
  if (io::existsFolder(pathmodel)) //load views, poses and self-occlusions
  {
    model.view_filenames_ = io::getFilesInDirectory(pathmodel, ".*view.*.pcd", false);
  ...
  else
  {
    ...
    for (size_t cam_id = 0; cam_id < cam_positions.size (); cam_id++)
    {
      ...
      io::writeMatrixToFile( path_pose.str (), final_mat);
...
template<typename Full3DPointT, typename PointInT, typename OutModelPointT>
void
PartialPCDSource<Full3DPointT, PointInT, OutModelPointT>::loadInMemorySpecificModelAndView(const std::string & dir, ModelT & model, int view_id)
{
  ...
  Eigen::Matrix4f pose = io::readMatrixFromFile( pathmodel.str () + "/" + file_replaced1 );
...
template<typename Full3DPointT, typename PointInT, typename OutModelPointT>
void
PartialPCDSource<Full3DPointT, PointInT, OutModelPointT>::assembleModelFromViewsAndPoses(ModelT & model,
                                   std::vector<Eigen::Matrix4f, Eigen::aligned_allocator<Eigen::Matrix4f> > & poses,
                                   std::vector<pcl::PointIndices> & indices,
...
template<typename Full3DPointT, typename PointInT, typename OutModelPointT>
void
PartialPCDSource<Full3DPointT, PointInT, OutModelPointT>::loadInMemorySpecificModel(ModelT & model)
{
  ...
  for (size_t i = 0; i < model.view_filenames_.size (); i++)
  {
    ...
    model.poses_[i] = io::readMatrixFromFile( pose_fn);

    std::string entropy_fn (view_file);
    boost::replace_last (entropy_fn, "view", "entropy");
    boost::replace_last (entropy_fn, ".pcd", ".txt");
    if(io::existsFile(entropy_fn))
        io::readFloatFromFile (entropy_fn, model.self_occlusions_[i]);
    else
...
//template class V4R_EXPORTS PartialPCDSource<struct pcl::PointXYZRGBNormal, struct pcl::PointXYZRGB, struct pcl::PointXYZRGB>;
//template class V4R_EXPORTS PartialPCDSource<struct pcl::PointXYZRGBNormal, struct pcl::PointXYZ, struct pcl::PointXYZ>;
//template class V4R_EXPORTS PartialPCDSource<struct pcl::PointXYZRGBNormal, struct pcl::PointXYZRGBA, struct pcl::PointXYZRGBA>;
//template class V4R_EXPORTS PartialPCDSource<struct pcl::PointNormal, struct pcl::PointXYZ, struct pcl::PointXYZ>;
}
