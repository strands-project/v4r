// SLICO.cpp: implementation of the SLICO class.
//
...
void SLICO::DoRGBtoLABConversion(const cv::Mat_<cv::Vec3b> &im_rgb, double*& lvec, double*& avec, double*& bvec)
{
	int sz = m_width*m_height;
  lvec = new double[sz];
  avec = new double[sz];
  ...
  for (int i=0; i<sz; i++)
  {
    const cv::Vec3b &rgb = im_rgb(i);
    RGB2LAB( rgb[0], rgb[1], rgb[2], lvec[i], avec[i], bvec[i] );
  }
}
 ...
 * DrawContoursAroundSegments
 */
void SLICO::DrawContoursAroundSegments(cv::Mat_<cv::Vec3b> &im_rgb, const cv::Mat_<int> &labels, int r, int g, int b)
{
	const int dx8[8] = {-1, -1,  0,  1, 1, 1, 0, -1};
	const int dy8[8] = { 0, -1, -1, -1, 0, 1, 1,  1};
...
//===========================================================================
void SLICO::PerformSuperpixelSLIC(
	vector<double>&				kseedsl,
	vector<double>&				kseedsa,
	vector<double>&				kseedsb,
	vector<double>&				kseedsx,
...
///	Save labels in raster scan order.
//===========================================================================
void SLICO::SaveSupervoxelLabels(
	const int**&				labels,
	const int&					width,
	const int&					height,
	const int&					depth,
...
//===========================================================================
void SLICO::EnforceSupervoxelLabelConnectivity(
...
//===========================================================================
void SLICO::DoSuperpixelSegmentation_ForGivenSuperpixelSize(
    const unsigned int* ubuff,
 ...
 * DoSuperpixelSegmentation_ForGivenSuperpixelSize
 */
void SLICO::DoSuperpixelSegmentation_ForGivenSuperpixelSize(const cv::Mat_<cv::Vec3b> &im_rgb,
        cv::Mat_<int> &labels, int &numlabels, const int &superpixelsize, const double& compactness)
{
  ...
  //------------------------------------------------
	...
	for( int s = 0; s < sz; s++ ) klabels[s] = -1;
  //--------------------------------------------------
  DoRGBtoLABConversion(im_rgb, m_lvec, m_avec, m_bvec);
	//--------------------------------------------------
  bool perturbseeds(false);//perturb seeds is not absolutely necessary, one can set this flag to false
	if(perturbseeds) DetectLabEdges(m_lvec, m_avec, m_bvec, m_width, m_height, edgemag);
	GetLABXYSeeds_ForGivenStepSize(kseedsl, kseedsa, kseedsb, kseedsx, kseedsy, STEP, perturbseeds, edgemag);
...
/// The labels can be saved if needed using SaveSuperpixelLabels()
//===========================================================================
void SLICO::DoSuperpixelSegmentation_ForGivenNumberOfSuperpixels(
    const unsigned int* ubuff,
	  const int	&width, const int	&height,
	  int*&	klabels,
	  int& numlabels,
 ...
 * DoSuperpixelSegmentation_ForGivenNumberOfSuperpixels
 */
void SLICO::DoSuperpixelSegmentation_ForGivenNumberOfSuperpixels(const cv::Mat_<cv::Vec3b> &im_rgb,
        cv::Mat_<int> &labels, int& numlabels, const int& K, const double& compactness)
{
  ...
  DoSuperpixelSegmentation_ForGivenSuperpixelSize(im_rgb, labels, numlabels, superpixelsize, compactness);
}
...
//===========================================================================
void SLICO::DoSupervoxelSegmentation(
