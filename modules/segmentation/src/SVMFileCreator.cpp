 ...
 */
void SVMFileCreator::setRelations(std::vector<v4r::Relation> _relations)
{
  relations = _relations;
  have_relations = true;
}

void SVMFileCreator::setSurfaces(std::vector<v4r::SurfaceModel::Ptr> _surfaces)
{
  surfaces = _surfaces;
  have_surfaces = true;
}
  ...
  if(!testset)
  {
    ...
    for(unsigned int i = 0; i < relations.size(); i++)
    {
      ...
      if((surfaces.at(relations.at(i).id_0)->indices.size() > min_size_1st) &&
         ...
         (relations.at(i).type == v4r::STRUCTURAL_RELATIONS))
      {
      ...
      if((surfaces.at(relations.at(i).id_0)->indices.size() > min_size_2nd) &&
         ...
         (relations.at(i).type == v4r::ASSEMBLY_RELATIONS))
      {
        usedRelations.at(i) = true;
      }
    }
    ...
    for(unsigned int i = 0; i < relations.size(); i++)
    {
      ...
      if( (relations.at(i).type ==v4r::STRUCTURAL_RELATIONS) && (relations.at(i).groundTruth != -1) )
      {
        ...
        else 
	{
      ...
      // assembly relations
      else if( (relations.at(i).type == v4r::ASSEMBLY_RELATIONS) && (relations.at(i).groundTruth != -1) )   
      {
	...
	fprintf(features_file_as,"\n");
      ...
      if( (relations.at(i).type != v4r::STRUCTURAL_RELATIONS) && (relations.at(i).type != v4r::ASSEMBLY_RELATIONS) )
      {
