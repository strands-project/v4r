...
#include "v4r/segmentation/Relation.h"
...
#include <v4r/segmentation/sequential_fitter.h>
...
inline std::vector<v4r::SurfaceModel::Ptr>
deepCopy(const std::vector<v4r::SurfaceModel::Ptr> &_surfaces) {
  std::vector<v4r::SurfaceModel::Ptr> surfaces;
  for(unsigned i=0; i<_surfaces.size(); i++) {
    v4r::SurfaceModel::Ptr c_surface;
    c_surface.reset(new v4r::SurfaceModel());
...
inline std::vector<v4r::SurfaceModel::Ptr>
deepCopyType(const std::vector<v4r::SurfaceModel::Ptr> &_surfaces, int type) {
  std::vector<v4r::SurfaceModel::Ptr> surfaces;
  for(unsigned i=0; i<_surfaces.size(); i++) {
