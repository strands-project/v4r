...
#include "v4r/attention_segmentation/ep_segmentation.h"

namespace v4r
{

EP_Segmenter::EP_Segmenter()
{
  have_cloud = false;
  have_cloud_l = false;
...
void EP_Segmenter::setPointCloud(pcl::PointCloud<pcl::PointXYZRGB>::Ptr _pcl_cloud)
{
  pcl_cloud = _pcl_cloud;
  have_cloud = true;
}

void EP_Segmenter::setPointCloud(pcl::PointCloud<pcl::PointXYZRGBL>::Ptr _pcl_cloud_l)
{
  pcl_cloud_l = _pcl_cloud_l;
  have_cloud_l = true;
}

void EP_Segmenter::setSaliencyMaps(std::vector<cv::Mat> _saliencyMaps)
{
  saliencyMaps = _saliencyMaps;
  have_saliencyMaps = true;
}

void EP_Segmenter::calculateNormals()
{
  // calcuate normals
...
void EP_Segmenter::calculatePatches()
{
  v4r::ClusterNormalsToPlanes::Parameter param;
  param.adaptive = true;         // use adaptive thresholds
  param.epsilon_c = 0.58;//0.62;//
  param.omega_c = -0.002;

  clusterNormals = v4r::ClusterNormalsToPlanes::Ptr(new v4r::ClusterNormalsToPlanes(param));
  // adaptive clustering
...
void EP_Segmenter::initModelSurfaces()
{
  ...
  v4r::SurfaceModeling::Parameter sfmParams;
  ...
  surfModeling = v4r::SurfaceModeling::Ptr(new v4r::SurfaceModeling(sfmParams));
...
void EP_Segmenter::modelSurfaces()
{
...
void EP_Segmenter::preComputeRelations()
{ 
...
void EP_Segmenter::computeRelations()
{
  structuralRelations.setSurfaces(surfaces);
  //   structuralRelations.setRelations(relations);
  structuralRelations.setNeighbours2D(ngbr2D_map);
...
void EP_Segmenter::graphBasedSegmentation()
{
...
bool EP_Segmenter::checkSegmentation(cv::Mat &mask, int originalIndex, int salMapNumber)
{ 
...
void EP_Segmenter::createTrainFile()
{
...
void EP_Segmenter::attentionSegment()
{
...
v4r::TimeEstimationClass timeEstimationClass_All(CLOCK_THREAD_CPUTIME_ID);//CLOCK_THREAD_CPUTIME_ID);//CLOCK_PROCESS_CPUTIME_ID
...
v4r::TimeEstimationClass timeEstimationClass_CustomLoopSegment(CLOCK_THREAD_CPUTIME_ID);
timeEstimationClass_CustomLoopSegment.countingStart();
    
v4r::TimeEstimationClass timeEstimationClass_CustomLoop(CLOCK_THREAD_CPUTIME_ID);
...
int EP_Segmenter::attentionSegment(cv::Mat &object_mask, int originalIndex, int salMapNumber)
{
...
void EP_Segmenter::attentionSegmentInit()
{
  if( (!have_cloud) || (!have_saliencyMaps) )
  {
    char* error_message = new char[200];
...
bool EP_Segmenter::attentionSegmentNext()
{
  assert(masks.size() == 1);
...
/*void EP_Segmenter::attentionSegment(int &objNumber)
{
  if( (!have_cloud) || (!have_saliencyMaps) )
  {
    char* error_message = new char[200];
...
v4r::TimeEstimationClass timeEstimationClass_CustomLoopTemp(CLOCK_THREAD_CPUTIME_ID);
